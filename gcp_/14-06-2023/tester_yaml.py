...
class TestEndpoints():
    ...
    def execute_tests(self, tests):
        rv = {}
        for t in tests:
            res = self.execute_test(t)
            rv[t['title']] = res

            err = res["error"]
            err_str = ErrorCodes.get_error(err)

            print(f'{t["title"]} -> {err_str}')

            if err != ErrorCodes.OK:
                print(f'  Expected RC : {res["expected_sc"]}')
                print(f'  Received RC : {res["response_sc"]}')
                print(f'  URL         : {res["request_url"]}')
                print(f'  Method      : {res["request_method"]}')
                print()
